...
Table table_init(void *base, Size n_bytes, Size n_bytes_per_row) {
  ...
  // TODO: Put these checks by in optionally with a param (maybe add a table
  // name too) ensure(n_bytes_per_row % 4 == 0, "Mis-aligned table row size");
...
Tab _tab_subset(Tab *src, Index row_i, Size n_rows, char *file, int line) {
  ensure_only_in_debug(n_rows >= 0, "tab_subset @%s:%d has illegal n_rows %lu",
                       file, line, n_rows);
...
void _tab_set(Tab *tab, Index row_i, void *src, char *file, int line) {
  ensure_only_in_debug(
      ...
      "tab_set outside bounds @%s:%d row_i=%lu n_rows=%lu n_bytes_per_row=%lu",
      file, line, row_i, tab->n_rows, tab->n_bytes_per_row);
  if (src != (void *)0) {
...
Index _tab_add(Tab *tab, void *src, pthread_mutex_t *lock, char *file,
  ...
  ensure_only_in_debug(tab->flags & TAB_GROWABLE,
                       "Attempting to grow to a un-growable table @%s:%d", file,
                       line);
  if (lock)
    pthread_mutex_lock(lock);
