...
def region_map(im, func, divs=4, include_coords=False, **kwargs):
    """
    ...
    results = []
    for win_im, _, _, coord in region_enumerate(im, divs):
        if include_coords:
            kwargs["coords"] = coord
        results += [func(win_im, **kwargs)]

    assert len(results) == divs * divs
    if isinstance(results[0], tuple):
        ...
        for ret_field_i in range(n_ret_fields):
            ...
            field_array = field.reshape((divs, divs, *field.shape[1:]))
            result_fields += [field_array]
        results = tuple(result_fields)
    else:
        results = np.array(results)
        results = results.reshape((divs, divs, *results.shape[1:]))
...
def gauss2_rho_form(amp, std_x, std_y, pos_x, pos_y, rho, const, mea):
    """
    ...
    cov = np.array(
        [[std_x ** 2, rho * std_x * std_y], [rho * std_x * std_y, std_y ** 2],]
    )
...
def fit_gauss2(im):
    ...
    def _gauss2(x, amp, std_x, std_y, pos_x, pos_y, rho, const):
        ...
        # debug(amp, std_x, std_y, pos_x, pos_y, rho)
