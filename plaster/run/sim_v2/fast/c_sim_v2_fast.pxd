...
cdef extern from "c_sim_v2_fast.h":
    ...
    ctypedef struct SimV2FastContext:
        ...
        Hash dyepep_hash
        c.Table pcbs
        c.Table pep_i_to_pcb_i
        c.RecallType *pep_recalls
        c.Size count_only
        c.Size output_n_dtrs
    ...
    c.Size dtr_n_bytes(c.Size n_channels, c.Size n_cycles)
    Hash hash_init(HashRec *buffer, c.Size n_max_recs)
    void context_work_orders_start(SimV2FastContext *ctx)
    c.Index context_dtr_get_count(SimV2FastContext *ctx, c.Index dtr_i)
    c.DyeType *context_dtr_dyetrack(SimV2FastContext *ctx, c.Index dtr_i)
