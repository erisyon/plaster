...
class PrepResult(BaseResult):
    ...
    @classmethod
    def random_fixture(cls, n_proteins):
        """
        ...
        pros = pd.DataFrame(
            dict(
                pro_id=["nul"] + [pro.name for pro in prep_params.proteins],
                pro_is_decoy=[False] + [False] * n_proteins,
                pro_i=list(range(n_proteins+1)),
                pro_ptm_locs=[None] + [None] * n_proteins,
                pro_report=[None] + [None] * n_proteins,
            )
        )

        # HERE How do I build this? Too tired.
