...
def zest_sigproc_v2_calibration():
    ...
    def background_stats():
        ...
        def _synth(_true_bg_std, chcy=None):
        ...
        def common_bg_stats():
            ...
            def it_subtracts_uniform_bg_mean_correctly():
                ...
                # is beacuse when we background subtract the mean should be
                # close to zero.
                im_sub = worker._background_subtraction(im, true_bg_mean)
        ...
        def it_adds_regional_bg_stats_to_calib_correctly():
            ...
            assert np.all(
                (true_bg_mean - 1 < est_bg_mean) & (est_bg_mean < true_bg_mean + 1)
            )

            est_bg_std = np.array(calib["regional_bg_std.instrument_channel[0]"])
            assert len(bg_std.shape) == 2
            assert np.all(
                (true_bg_std - 1 < est_bg_std) & (est_bg_std < true_bg_std + 1)
            )
        ...
        def it_estimates_nonuniform_bg_mean_correctly():
            ...
            dist_to_edge = np.min(xs, im.shape[0] - xs, ys, im.shape[1] - ys)
            scale_factor = np.ones_like(im)
            scale_factor[dist_to_edge < border] = 0.5
            # TODO: REmove this comment once we confirm above np code is equiv.
            ...
            for y, x in grid_walk(divs):
                if x in [0, divs - 1] or y in [0, divs - 1]:
                    assert np_within((bg_mean * 0.5), bg_mean[y, x], bg_std)
                    assert np_within(bg_std * 0.5, bg_std[y, x], bg_std / 3)
                else:
                    assert np_within(bg_mean, bg_mean[y, x], bg_std)
                    assert np_within(bg_std, bg_std[y, x], bg_std / 3)
        ...
        def it_estimates_nonuniform_bg_std_correctly():
            ...
            with synth.Synth(overwrite=True) as s:
                ...
                synth.CameraModel(bias=bg_mean, std=tgt_std2)
            ...
            for x in range(0, divs):
                for y in range(0, divs):
                    if x in [0, divs - 1] or y in [0, divs - 1]:
                        ...
                        assert np_within(bg_mean[x][y], bg_mean, tgt_std2)
                    else:
                        assert np_within(bg_std[x][y], bg_std, bg_std / 2)
                        assert np_within(bg_mean[x][y], bg_mean, bg_std)
            return True
        ...
        def it_subtracts_nonuniform_bg_mean_correctly():
            ...
            assert np_within(np.mean(new_mean), 0, bg_std)
            return True
