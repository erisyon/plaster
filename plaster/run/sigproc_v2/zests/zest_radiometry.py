...
def zest_radiometry():
    ...
    def it_finds_many_locs_with_jitter():
        ...
        locs = [
            (y, x)
            for y in np.linspace(10, 120, 8)
            for x in np.linspace(10, 120, 8)
        ]
        locs = np.array(locs)
        locs = locs + np.random.uniform(-1, 1, size=locs.shape)
        for loc in locs:
            imops.accum_inplace(chcy_ims[0, 0], _peak(loc[1], loc[0], 128), loc=XY(0,0), center=False)

        radmat = radiometry_field_stack(chcy_ims, locs=locs, reg_psf=reg_psf, focus_adjustment=np.ones((n_cycles)))
        # print(f"{y:8.2f} {x:8.2f} {radrow[0,0,0,0]:8.2f} {radrow[0,0,0,1]:8.2f}")
    ...
    def it_finds_changes_over_cycles():
        ...
        imops.accum_inplace(chcy_ims[0, 0], _peak(x, y, 11, 1000), loc=XY(0, 0), center=False)
        imops.accum_inplace(chcy_ims[0, 1], _peak(x, y, 11, 900), loc=XY(0, 0), center=False)
        imops.accum_inplace(chcy_ims[0, 2], _peak(x, y, 11, 100), loc=XY(0, 0), center=False)
    ...
    def it_uses_appropriate_regional_psf():
        ...
        for loc, peak_w, peak_rho in zip(locs, peak_ws, peak_rhos):
            peak_im = imops.gauss2_rho_form(1000.0, peak_w, 1.8, loc[1], loc[0], peak_rho, 0.0, 128)
    ...
    def it_adjusts_for_focus():
        ...
        for cy_i, focus in enumerate(focuses):
            for loc, peak_w, peak_rho in zip(locs, peak_ws, peak_rhos):
                peak_im = imops.gauss2_rho_form(1000.0, focus*peak_w, focus*1.8, loc[1], loc[0], peak_rho, 0.0, 128)
                imops.accum_inplace(chcy_ims[0, cy_i], peak_im, loc=XY(0, 0), center=False)

        radmat = radiometry_field_stack(chcy_ims, locs=locs, reg_psf=reg_psf, focus_adjustment=np.array(focuses))
