...
class CallBag:
    ...
    def fdr_from_decoys(self,probability_wrong_call_is_decoy=0.5):
        """
        Use decoy count and probability to compute FDR
        Returns:
            np.ndarray of fdr values
        """
        peps = self._prep_result.pros__peps__pepstrs()
        ...
        # Merge with protein info
        sorted_df = self.df.merge( peps, left_on="pred_pep_iz", right_on="pep_i").sort_values("scores", ascending=False)

        # Add Fdrs
        fdr = np.cumsum(sorted_df.pro_is_decoy.values) / np.arange( 1, len(sorted_df.index) + 1 )
    ...
    score.
    If this relationship holds, and our biological-sample modeling is sound (e.g. there 
    aren't grossly incorrect abundances or contaminants that are being classified as 
    our peptides-of-interest)
    
    a) ratio of individual_peptide_FDR to total_set_FDR in test data
    b) score thresholds for individual_peptide_FDR in test data
    
    If we have done a good job of ensuring our FDR results on "largely the same" results 
