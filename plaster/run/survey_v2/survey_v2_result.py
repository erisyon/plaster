...
class SurveyV2Result(BaseResult):
    ...
    survey_filter_schema = s(
        s.is_kws_r(
            ...
            exclude_runs=s.is_list(s.is_str()),
            include_runs=s.is_list(s.is_str()),
            max_dyes_per_ch=s.is_int(noneable=True),
            max_pep_len=s.is_int(noneable=True),
            max_ptms_per_pep=s.is_int(noneable=True),
            multi_peptide_metric=s.is_str(
                noneable=True, options=["dist_avg", "dist_min"]
            ),
    ...
    def _domain_loss(self, df, filters, msg):  # debug aid
        if filters.verbose:
            ...
            if domain_loss:
                print(f"  {filters.objective} {msg} domain_loss: {sorted(domain_loss)}")
    ...
    def _apply_filters(self, filters, prep=None):
        ...
        if filters is not None:
            ...
            if filters.poi_only == True:
                ...
                poi_iz = prep.pros__in_report().pro_i.values
                if len(poi_iz) > 0:
    ...
    def protein_coverage(self, prep_result, filters=None, df=None):
        ...
        n_poi = prep_result.n_pros_of_interest
        poi_iz = (
            prep_result.pros__in_report().pro_i.values
            if n_poi > 0
